...
def movies_db_refactor():
    # movies manipulation
    df = pd.read_csv('title.basics.tsv', sep='\t', index_col="tconst")
...
def people_db_refactor():
    # people manipulation
    df = pd.read_csv('name.basics.tsv', sep='\t', index_col="nconst")
...
def crew_db_refactor():
    # crew manipulation
    df = pd.read_csv('title.crew.tsv', sep='\t', index_col="tconst")
...
def ratings_db_refactor():
    ...
    df = pd.read_csv('title.ratings.tsv', sep='\t', index_col="tconst")
...
def roles_db_refactor():
    # roles manipulation
    df = pd.read_csv('title.principals.tsv', sep='\t', index_col="tconst")
    df = df.drop(['job', 'characters', 'ordering'], axis=1)
    df = df.query('category == ["actor", "actress", "director"]')
    df['category'] = df['category'].str.replace({'actress': 'actor'})
